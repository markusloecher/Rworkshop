...
borrowed from: https://www.kaggle.com/notaapple/detailed-exploratory-data-analysis-using-r
...
library(readr)
train <- read_csv("C:/DatenLoecher/kaggle/HousePrices/data/train.csv.gz")
test <- read_csv("C:/DatenLoecher/kaggle/HousePrices/data/test.csv.gz")
```
...
benchFit = lm(SalePrice ~ LotArea, data = train)
summary(benchFit)
preds = predict(benchFit, test)
subMiss = cbind.data.frame(Id= test$Id,SalePrice= preds)
write.csv(subMiss, "C:/DatenLoecher/kaggle/HousePrices/data/submission1.csv", quote=F, row.names = FALSE)
```


## Tasks in class

1. reproduce benchmark from a linear regression on year and month of sale, lot square footage, and number of bedrooms and submit
```{r}
benchFit2 = lm(SalePrice ~ BedroomAbvGr + factor(YrSold) + factor(MoSold) + LotArea, data = train)
summary(benchFit2)
preds = predict(benchFit2, test)
#remove NAs:
jj = is.na(preds)
preds[jj] = mean(train$SalePrice)


subMiss = cbind.data.frame(Id= test$Id,SalePrice= preds)
write.csv(subMiss, "C:/DatenLoecher/kaggle/HousePrices/data/submission2.csv", quote=F, row.names = FALSE)


```

2. expand model with variables that you deem "worthwhile"
3. Using cross validation find the best 
  * nearest neighbor model
  * GAM
4. I want to see at least 4 submissions.
