...
fit1=rpart(Survived ~ Sex + Age + Pclass,train)
plot(as.party(fit1))

set.seed(123)
for ( i in 1:3) {
  fit1=rpart(Survived ~ Sex + Age + Pclass,train[sample(1:nrow(train), nrow(train)/2),])
  plot(as.party(fit1));#title("half data")
}
...
data(Boston)
par(mfrow=c(2,2))
fit2=rpart(medv~.,Boston)
plot(as.party(fit2));#title("all data")

for ( i in 1:3) {
  fit2=rpart(medv~.,Boston[sample(1:nrow(Boston), nrow(Boston)/2),])
  plot(as.party(fit2));#title("half data")
}
...
N=100;x=list()
#true parameters:
mu=10;s=3;xx=mu + 3.5*seq(-s,s,length=100);
xx2=mu + 3.5*seq(-s,s,length=N/10);dx=mean(diff(xx2));
#xx3=mu + 3.5*seq(-s,s,length=N/10)/sqrt(N)

par(mfrow=c(3,3),mar=rep(2,4), cex=1.75)
for (i in 1:3) {
  x[[i]] = rnorm(N,mu,s)
  br = hist(x[[i]],main="", xlim = mu+3.5*c(-s,s), breaks = xx2);grid()
  abline(v=c(mu,mean(x[[i]])), col = c("red", "green"))
  #lines(xx, N*dx*dnorm(xx,mu,s),col=2,lwd=1.5)
}
B=500
REPL=c(TRUE,TRUE)
for (j in 1:2) {
  for (i in 1:3) {
    ...
    ym = vector()
    for (k in 1:B) {
      y = sample(x[[i]],N,replace= REPL[j])
      ym[k] = mean(y)
    }
    xx3=xm + 3.5*seq(-s,s,length=N/10)/sqrt(N)
    hist(ym,main="", xlim = xm+3.5*c(-s,s)/sqrt(N), breaks=xx3);grid()
    abline(v=c(mu,xm), col = c("red", "green"))
    xx3=xm + 3.5*seq(-s,s,length=N)/sqrt(N)
    lines(xx3, B*dx/sqrt(N)*dnorm(xx3,xm,s/sqrt(N)),col=2,lwd=1.5)
    #text("topright", labels=paste("s=",round(sd(ym),1)))
  }
}
...
#down sample:
set.seed(1)
#Default = Default[sample(10^4, 10^3),]
  
library(boot)
B=500#too small but good for Rmd compilation


boot.fn=function(data,index)
  coefficients(lm(medv~ rm +lstat +crim+dis,data=data,subset=index))
 #coefficients(lm(mpg~horsepower+I(horsepower^2),data=data,subset=index))
#set.seed(1)
boot.fn(Boston,1:nrow(Boston))
boot.fn(Boston,sample(506))
boot(Boston,boot.fn,B)
summary(lm(medv~ rm +lstat +crim+dis,data=Boston))$coef
...
Write code which 
(i)   "bootstraps your data" (samples N times with replacement)
(ii)  estimate model on these data
(iii) predict on the left-out ("out-of-bag")

This is the last Codeathon of 2016
...
B = 500
OOBpreds = matrix(NA,ncol=B,nrow=N)
for (i in 1:500){
  #(i)   "bootstraps your data" (samples N times with replacement)
  xBoot = sample(N,N,TRUE)
  #(ii)  estimate model on these data
  InBagFit[[i]] =rpart(Survived ~ Sex + Age + Pclass, train, subset=xBoot)
  #(iii) predict on the left-out ("out-of-bag")
  OOBindex = (1:N)[-unique(xBoot)]
  OOBdata = train[OOBindex,]
  #OOBdata = train[which(!(1:N) %in% xBoot),]
  OOBpreds[OOBindex,i]=predict(InBagFit[[i]],newdata=OOBdata, type="class")
  #passID[[i]] = OOBindex
  #AlignedPreds[[i]] =cbind(OOBpreds[[i]],passID[[i]])
  
}
#1 Solution; unlist

BaggedPreds = rowMeans(OOBpreds, na.rm=TRUE) > 1.5


# AllPreds = unlist(OOBpreds)
# AllIDs = unlist(passID)
# 
# 
# for (i in 1:500){
#   preds=OOBpreds[[i]]
...
Bagged trees are already forests but we need one more ingredient to make them **random**.
...
fit = randomForest(Survived ~ Sex + Age + Pclass, na.omit(train), ntrees= 500, mtry=2)

# can we get an estimate of quality/performance ??
# it comes for free !!!
